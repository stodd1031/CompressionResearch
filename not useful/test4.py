...
orderRight = order(rightArr[0:size])
...
def checkRight(array):
    rightArrCpy = rightArr[0:size]
    for index in range(0, size):
        if array[index] in rightArrCpy:
            rightArrCpy[rightArrCpy.index(array[index])] = -1
        else:
            return 0
    return 1
...
def rec(current):
    ...
    if current == size-1:
        ...
        if checkRight(testRightArr[0:size]):
            pass
            print(testLeftArr)
            print(testBotArr)
            print(testRightArr)
...
print("input 3", (total.bit_length())*3)
